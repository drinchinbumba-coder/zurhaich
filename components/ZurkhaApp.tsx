  ...
  return (
    <Card glow style={{ textAlign: "center", padding: "44px 28px" }}>
      <div style={{ width: 64, height: 64, margin: "0 auto 16px" }}>
        <img src="/zurhaich-logo.png" alt="Зурхайч" style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 0 12px rgba(227,180,88,0.5))" }} />
      </div>
...
export default function ZurkhaApp() {
  ...
  useEffect(() => {
    (async () => {
      const [auth, saved, prem, partnerSavedData] = await Promise.all([
        store.get<boolean>("isAuthenticated"),
        store.get<Profile>("profile"),
        store.get<boolean>("isPremium"),
        store.get<PartnerProfile>("partner"),
      ]);
      if (saved) { setProfile(saved); setProfileSaved(true); }
      if (prem) setIsPremium(true);
      if (partnerSavedData) { setPartner(partnerSavedData); setPartnerSaved(true); }
  ...
  const handleAuthComplete = useCallback(
    async (_provider: string, prof: Profile, remember: boolean) => {
      if (remember) {
  ...
  return (
    ...
    >
      ...
      {/* Нэвтрэлтийн байдал шалгаж байна */}
      {isAuthenticated === null && (
        <div style={{
          minHeight: "100vh", display: "flex", alignItems: "center",
          justifyContent: "center", flexDirection: "column", gap: 20,
        }}>
          <div style={{ width: 60, height: 60, animation: "spin 1.5s linear infinite" }}>
            <img src="/zurhaich-logo.png" alt="Зурхайч" style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 0 14px rgba(227,180,88,0.7))" }} />
          </div>
          <div style={{ color: "rgba(227,180,88,0.6)", fontSize: 13, letterSpacing: 1 }}>
            Ачааллаж байна...
          </div>
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div className="hdr-logo-icon" style={{ width: 36, height: 36, flexShrink: 0 }}>
              <img
                src="/zurhaich-logo.png"
                alt="Зурхайч"
                style={{ width: "100%", height: "100%", objectFit: "contain", filter: `drop-shadow(0 0 10px ${C.gold}88)` }}
              />
            </div>
            <div>
              <div
